...
class PlayOtherGames():
    ...
    async def start_game_dice(self, value_dice1, value_dice2):
        ...
        if value_dice1[0] > value_dice2[0]:
            ...
            #self.winner_firstname = self.player_1_firstname
            ...
            #self.winner_firstname = self.player_2_firstname
        ...
        winner_info = await bot.get_chat(self.winner)
        print(winner_info.first_name)
        await bot.send_message(config('chat_id'),f"{other_games_info[self.game_name]['text']} #{self.game_id}\n\n💰 Выигрыш: {self.win_money}₽\n\n👥Игроки:\n1️⃣ - @{self.player_1_firstname}[{value_dice1[0]}]\n2️⃣ - @{self.player_2_firstname}[{value_dice2[0]}]\n\n🥳 Победитель: @{winner_info.first_name}")
